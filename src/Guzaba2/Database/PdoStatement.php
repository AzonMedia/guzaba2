...
final class PdoStatement extends Statement
{
    ...
    public function execute($params = [], bool $buffered_query = TRUE, bool $disable_sql_cache = FALSE, bool $enforce_DQL_statements_only = TRUE): self
    {
        ...
        $current_db_transaction = self::TransactionManager()->getCurrentTransaction($this->connection);
        ...
        if (self::ENABLE_SELECT_CACHING && !$disable_sql_cache) {
            ...
            if ($statement_group_type == statementTypes::STATEMENT_GROUP_DQL) {
                ...
                if (self::TransactionManager()->getCurrentTransaction($this->connection) && self::INVALIDATE_SELECT_CACHE_ON_COMMIT) {
        ...
        try {
            ...
            if ($sqlstate == 'HY000') { //Prepared statement needs to be re-prepared
                if ($errorcode == 1205) {
                    ...
                    $current_transaction = self::TransactionManager()->getCurrentTransaction($this->get_connection());
